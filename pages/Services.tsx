...
import { PaintBucket, Hammer, Ruler, Zap, Droplet, Truck, CheckCircle2, X, ArrowLeft, Phone, User as UserIcon, FileText, Loader2 } from 'lucide-react';
...
const Services: React.FC = () => {
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    try {
        ...
        alert('خطا در ثبت سفارش. لطفا مجددا تلاش کنید.');
    } finally {
        setSubmitting(false);
    }
  };
              ...
              {formStep === 'details' ? (
                <form onSubmit={handleSubmitOrder} className="space-y-4">
                  ...
                  <button 
                    type="submit"
                    disabled={submitting}
                    className={`w-full py-4 rounded-xl font-bold text-white shadow-md bg-gradient-to-r ${selectedService.color} hover:opacity-90 transition-all mt-4 flex justify-center items-center gap-2`}
                  >
                    {submitting ? <Loader2 className="animate-spin" /> : 'ارسال درخواست رایگان'}
                  </button>
                </form>
              ) : (
